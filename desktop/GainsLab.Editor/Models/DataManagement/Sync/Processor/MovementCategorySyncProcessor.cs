...
public class MovementCategorySyncProcessor : ISyncEntityProcessor
{
    ...
    public async Task<Result> ApplyAsync(IReadOnlyList<ISyncDto> items, ILocalRepository localRepository, CancellationToken ct)
    {
          ...
          try
        {
            ...
            var categoryLookup = new Dictionary<Guid, MovementCategoryDTO>();
            ...
            foreach (var dto in typed)
            {
                ...
                if (entity is null)
                {
                    entity = new MovementCategoryDTO
                    {
                        Name = dto.Name,
                        GUID = dto.GUID,
    ...
    private async Task SyncParentCategoriesAsync(GainLabSQLDBContext dbContext, MovementCategoryDTO category, Guid? parentGuid ,IReadOnlyList<Guid> baseCategories, CancellationToken ct)
    {
        if (category.Id == 0)
        {
        ...
        foreach (var guid in missingGuids)
        {
            ...
            dbContext.MovementCategoryRelations.Add(new MovementCategoryRelationDTO()
            {
