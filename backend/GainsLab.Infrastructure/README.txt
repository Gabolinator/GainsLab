...
- Postgres migrations reside in the root `Migrations/` folder.
...
2. When adding new entity types, implement the corresponding handler, mapper, and cache.
3. Prefer pure EF Core code—avoid direct SQL unless necessary, and keep migrations in sync with changes.
4. Verify seeds and migrations on both SQLite and Postgres to avoid drift between environments.
