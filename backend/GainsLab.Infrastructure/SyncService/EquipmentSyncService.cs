...
public class EquipmentSyncService : ISyncService<EquipmentSyncDTO>
{
    ...
    /// </summary>
    public EquipmentSyncService(GainLabPgDBContext db, ILogger log)
    {
        _db = db; _log = log;
    }
    ...
    public async Task<PushResult> PushAsync(IEnumerable<EquipmentSyncDTO> items, CancellationToken ct)
    {
        ...
        var payloads = new List<EquipmentRecord>();
        foreach (var dto in items)
        {
            ...
            DescriptorRecord descriptorDto = GetDescriptorDto(dto.DescriptorGUID);
    ...
    private DescriptorRecord GetDescriptorDto(Guid? descriptorGuid)
    {
