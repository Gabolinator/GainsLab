...
public class MuscleSyncService : ISyncService<MuscleSyncDTO>
{
    ...
    public MuscleSyncService(GainLabPgDBContext db, ILogger log)
    {
        _db = db;
        _log = log;
    }
    ...
    public async Task<PushResult> PushAsync(IEnumerable<MuscleSyncDTO> items, CancellationToken ct)
    {
        ...
        var payloads = new List<(MuscleRecord Entity, IReadOnlyList<Guid> Antagonists)>();
        ...
        try
        {
            await executionStrategy.ExecuteAsync(async () =>
            {
                ...
                var accepted = 0;
                var failed = 0;
                var pendingAntagonists = new List<(MuscleRecord Entity, IReadOnlyList<Guid> Antagonists)>();
    ...
    /// </summary>
    private async Task SyncAntagonistsAsync(MuscleRecord muscle, IReadOnlyList<Guid>? desiredGuids, CancellationToken ct)
    {
        if (muscle.Id == 0)
        {
        ...
        foreach (var guid in missingGuids)
        {
            ...
            _db.MuscleAntagonists.Add(new MuscleAntagonistRecord
            {
                MuscleId = muscle.Id,
