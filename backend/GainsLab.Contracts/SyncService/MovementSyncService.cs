...
public class MovementSyncService : ISyncService<MovementSyncDTO>
{
    ...
    public MovementSyncService(GainLabPgDBContext db, Core.Models.Core.Utilities.Logging.ILogger log)
    {
        _db = db;
        _log = log;
    }
    ...
    public async Task<PushResult> PushAsync(IEnumerable<MovementSyncDTO> items, CancellationToken ct)
    {
        ...
        var payloads = new List<(MovementDTO Entity, MovementSyncDTO Source, IReadOnlyList<Guid> Equipments, IReadOnlyList<Guid> PrimaryMuscles, IReadOnlyList<Guid> SecondaryMuscles)>();
        ...
        try
        {
            await executionStrategy.ExecuteAsync(async () =>
            {
                ...
                var accepted = 0;
                var failed = 0;
                var pendingRelations = new List<(MovementDTO Movement, IReadOnlyList<Guid> Equipments, IReadOnlyList<Guid> Primary, IReadOnlyList<Guid> Secondary)>();
    ...
    private async Task SyncEquipmentRelationsAsync(int movementId, IReadOnlyList<int> desiredEquipmentIds, CancellationToken ct)
    {
        ...
        foreach (var equipmentId in desiredEquipmentIds.Distinct())
        {
            ...
            _db.MovementEquipmentRelations.Add(new MovementEquipmentRelationDTO
            {
    ...
    private async Task SyncMuscleRelationsAsync(int movementId, IReadOnlyList<int> primaryIds, IReadOnlyList<int> secondaryIds, CancellationToken ct)
    {
        ...
        {
            _db.MovementMuscleRelations.Add(new MovementMuscleRelationDTO
            {
                MovementId = movementId,
                MuscleId = muscleId,
